...
Scene_Boot.prototype.start = function() {
    ...
    if (DataManager.isBattleTest()) {
        ...
        // Onakin custom
        Graphics.playVideo("movies/OP.webm");
        Graphics._video.volume = 0.12;
        Graphics._video.loop = false;
        // 5秒経ったらスキップできるように
        setTimeout(function() {
            canOpeningDemoEnd = true;
        }, 5000);
        // 72秒後にタイトルへ遷移
        setTimeout(function() {
            openingDemoEnd = true;
...
Scene_Boot.prototype.update = function() {
    if (canOpeningDemoEnd && (TouchInput.isTriggered() || Input.isTriggered("ok") || openingDemoEnd)) {
        canOpeningDemoEnd = false;
        openingDemoEnd = true;
        Graphics._video.opacity = 0;
        Graphics._video.volume = 0;
        // タイトルへ遷移
