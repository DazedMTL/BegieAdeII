...
Window_SavefileList.prototype.drawItem = function (index) {
  ...
  if (info) {
    this.changePaintOpacity(valid);
    this.drawContentsText(info, rect, valid);
    this.changePaintOpacity(true);
  }
};
...
Window_SavefileList.prototype.drawFileId = function (id, x, y) {
  ...
  this.drawText("BegieAde No." + id, x, y, 180);
};

Window_SavefileList.prototype.drawContentsText = function (info, rect, valid) {
  var bottom = rect.y + rect.height;
  rect.y += 35;
  if (info.inyokuExt) {
    // this.changeTextColor(this.textColor(4));
    // this.drawText('淫欲経験値合計', rect.x, rect.y, 130);
    // this.resetTextColor();
    // this.drawText(info.inyokuExt, rect.x + 135, rect.y, 60);
  }
  rect.y += 35;
  if (info.timestamp) {
    ...
    this.drawText(dateStr, rect.x, rect.y, 130);
  }
  rect.x += 210;
  rect.y -= 70;
  if (info.level) {
    this.changeTextColor(this.textColor(4));
    this.drawText("Lv.", rect.x, rect.y, rect.width - 192);
    this.resetTextColor();
    this.drawText(info.level, rect.x + 100, rect.y, rect.width - 192);
  }
  rect.y += 35;
  if (info.inyoku) {
    // this.changeTextColor(this.textColor(4));
    // this.drawText('淫欲度 ', rect.x, rect.y, rect.width - 192);
    // this.resetTextColor();
    // this.drawText(info.inyoku + '%', rect.x + 100, rect.y, rect.width - 192);
  }
  rect.y += 35;
  if (info.gold) {
    this.changeTextColor(this.textColor(4));
  ...
  // this.changeTextColor(this.textColor(4));
  // this.drawText('プレイ時間', rect.x, rect.y, 130);
  rect.y += 70;
  this.resetTextColor();
  this.drawText(info.playtime, rect.x + 30, rect.y, 130);
  // var lineHeight = this.lineHeight();
...
Window_SavefileList.prototype.drawContents = function (info, rect, valid) {
  var bottom = rect.y + rect.height;
  if (rect.width >= 420) {
    this.drawLevel(info, rect.x + 192, rect.y, rect.width - 192);
    if (valid) {
...
Window_SavefileList.prototype.drawLevel = function (info, x, y, width) {
  if (info.level) {
    this.drawText(info.level, x, y, width);
  }
};
...
Window_Message.prototype.initialize = function () {
  ...
  var height = this.windowHeight();
  var x = (Graphics.boxWidth - width) / 2;
  Window_Base.prototype.initialize.call(this, x, 0, width - 500, height);
  this.openness = 0;
  this.initMembers();
...
Window_Message.prototype.updatePlacement = function () {
  ...
  // hashcode: ウィンドウの位置ずらす
  //if(this._positionType === 0 || this._positionType === 1 || this._positionType === 2)
  //    this.x += 240;
};
