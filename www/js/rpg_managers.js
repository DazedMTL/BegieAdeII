...
DataManager.makeSavefileInfo = function() {
    ...
    info.inyokuExt  = $gameVariables.value(26);
    info.gold       = $gameParty.gold();
    info.saveCnt    = $gameSystem.saveCount();
    info.playtime   = $gameSystem.playtimeText();
    info.timestamp  = Date.now();
    info.level      = $gameParty.members()[0].level;
    info.state      = $gameParty.members()[0].isStateAffected(51);
    info.inyoku     = $gameVariables.value(3);
    return info;
};
...
AudioManager._beforeMeBgmVolume = 0;
AudioManager._isDownVolume = false;
AudioManager._nowMeName = null;
AudioManager._downPercentage = 3; // 1/3

/** ME再生＆BGM音量徐々に下げる */
AudioManager.playMe = async function(me) {
    if (me.name) {
        ...
        if (this._bgmBuffer && this._currentBgm) {
            // console.info('start volume down');
            AudioManager._nowMeName = me.name;
            this._meBuffer.addStopListener(this.fadeUpBgm.bind(this, me.name));
            if (this._isDownVolume) return; // 既に音量が下がっている場合はスキップ
            this._isDownVolume = true;
            // 現在のBGM音量を保存
            this._beforeMeBgmVolume = this._bgmBuffer.volume;
            const target = this._beforeMeBgmVolume / AudioManager._downPercentage;
            const interval = 100;
            const steps = 5;
            ...
            for (let count = 0; count < steps; count++) {
                ...
                this._bgmBuffer.volume -= diff;
                // console.info('volume down', this._bgmBuffer.volume);
            }
        }
    }
};
...
/** BGM音量を徐々に上げる */
AudioManager.fadeUpBgm = async function(name) {
    if (this._bgmBuffer && this._currentBgm && this._bgmBuffer.isPlaying()) {
        if (!this._isDownVolume) return; // 音量が下がっていない場合はスキップ
        await new Promise(resolve => setTimeout(resolve, 100)); 
        if (name !== AudioManager._nowMeName) return; // 現在再生中のMEと違う場合はスキップ
        // console.info('start volume up');
        const interval = 100;
        const steps = 5;
        const diff = this._beforeMeBgmVolume / (AudioManager._downPercentage * steps);
        // 5段階で音量を上げる
        for (let count = 0; count < steps; count++) {
            await new Promise(resolve => setTimeout(resolve, interval));
            this._bgmBuffer.volume += diff;
            if (count === steps - 1) {
                this._isDownVolume = false;
                AudioManager._nowMeName = null;
                // console.info('volume up', this._bgmBuffer.volume);
            }
        }
    }
};
