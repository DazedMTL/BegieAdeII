...
Moghunter.mcursor_float = String(
...
Moghunter.mcursor_animated = String(
  Moghunter.parameters["Frames Animation"] || "false"
);
Moghunter.mcursor_maxframes = Number(Moghunter.parameters["Max Frames"] || 4);
Moghunter.mcursor_animationSpeed = Number(
  Moghunter.parameters["Animation Speed"] || 7
);
...
Scene_Base.prototype.create_menucursor = function () {
  ...
  this._mcursor_data[11] = Math.min(
    Math.max(Moghunter.mcursor_animationSpeed, 1),
    999
  );
...
Scene_Base.prototype.update_mcursor = function () {
  ...
  if ($gameTemp._mcursorData[1] > 0) {
    ($gameTemp._mcursorData[1] -= 1), (this._sprite_mcursor.opacity += 15);
  } else {
    this._sprite_mcursor.opacity = 0;
  }
  ...
  var nx =
    $gameTemp._mcursorData[2] + this._mcursor_data[0] + this._mcursor_data[2];
  this._sprite_mcursor.x = this.cursorMoveToSVD(this._sprite_mcursor.x, nx, 5);
  this._sprite_mcursor.y = this.cursorMoveToSVD(
    this._sprite_mcursor.y,
    $gameTemp._mcursorData[3] + this._mcursor_data[1],
    5
  );
};
...
Scene_Base.prototype.set_mcursor_initial = function () {
  ...
  this._mcursor_data[1] =
    this._sprite_mcursor.bitmap.height / 2 + Moghunter.mcursor_y;
  if (this._mcursor_data[6]) {
    this._mcursor_data[7] = Math.floor(
      this._sprite_mcursor.bitmap.width / Moghunter.mcursor_maxframes
    );
...
// * Update Arrows
//==============================
var _mog_mcursor_wSelect_updateArrows =
...
Window_Selectable.prototype.set_mcursor_data = function () {
  ...
  if (
    rect.x < 0 ||
    rect.y < 0 ||
    rect.x > this.width - 48 ||
    rect.y > this.height - 48
  ) {
    return;
  }
