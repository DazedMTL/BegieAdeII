...
(function () {
      ...
      text = text.replace(
        ...
        function () {
          return $gameVariables.value(
            parseInt(wstringToString(arguments[1]), 10)
          );
        }.bind(this)
      );
      text = text.replace(
        ...
        function () {
          try {
            ...
            if (value != null) {
              ...
              console.log(
                "制御文字 \\JS のパラメータでエラー  詳細：評価値が無い(null or undefined)"
              );
              return 0;
            }
          } catch (ex) {
            console.log(
              "制御文字 \\JS のパラメータでエラー  詳細： " + ex.toString()
            );
            return 0;
          }
        }.bind(this)
  ...
  Game_Interpreter.prototype.executePluginCommand = function (command, args) {
    ...
    if (typeof this[methodName] === "function") {
      ...
      for (var i = 0; i < args.length; i++) {
        args[i] = Game_Interpreter.prototype.pluginCommandBook_unescape(
          args[i]
        );
      }
      try {
   ...
   *  呼び出し元アイテム取得 1    （変数0001にコモンイベントを呼び出したアイテムIDを入れる
   */
  Game_Interpreter.prototype.pluginCommandBook_呼び出し元アイテム取得 =
    function (args) {
   ...
   *  呼び出し元スキル取得 1    （変数0001にコモンイベントを呼び出したスキルIDを入れる
   */
  Game_Interpreter.prototype.pluginCommandBook_呼び出し元スキル取得 = function (
    args
  ) {
    if (
      eval(String(parameters["呼び出し元スキルの記録を使わない"] || "false"))
    ) {
      window.alert(
        "「呼び出し元スキル取得」を使うにはプラグインマネージャーから「TkoolMV_PluginCommandBook.js」の「呼び出し元スキルの記録を使わない」を「はい」してください"
      );
      return;
    }
    var varId = parseInt(args[0]); // 変数ID
    ...
    if ($gameParty.inBattle()) {
      // console.log(BattleManager._subject.lastBattleSkill());
  ...
  /* エネミー側にも同様の機能を追加 */
  Game_Enemy.prototype.lastBattleSkill = Game_Actor.prototype.lastBattleSkill;
  Game_Enemy.prototype.setLastBattleSkill =
  ...
  Game_Enemy.prototype.selectAllActions = function (actionList) {
    _Game_Enemy_selectAllActions.apply(this, arguments);
    this.setLastBattleSkill(this.action(0).item());
  };
  ...
  Game_Interpreter.prototype.pluginCommandBook_レベルの変更 = function (args) {
    ...
    var show = { 表示: true, 非表示: false, はい: true, いいえ: false }[
      args[2] || "表示"
    ];
  ...
  Game_Interpreter.prototype.pluginCommandBook_バイブレーション = function (
    args
  ) {
  ...
  Game_Interpreter.prototype.pluginCommandBook_指定位置の通行判定取得 =
    function (args) {
  ...
  Game_Interpreter.prototype.pluginCommandBook_Get_Location_Pass = function (
    args
  ) {
    this.pluginCommandBook_指定位置の通行判定取得(args);
  };

  Game_Interpreter.prototype.pluginCommandBook_スイッチの初期化 = function (
    args
  ) {
    var exceptionValues = [];
  ...
  Game_Interpreter.prototype.pluginCommandBook_Init_Variables = function (
    args
  ) {
    this.pluginCommandBook_変数の初期化(args);
  };

  Game_Interpreter.prototype.pluginCommandBook_セルフスイッチの初期化 =
    function (args) {
      $gameSelfSwitches.clear();
    };
  Game_Interpreter.prototype.pluginCommandBook_Init_Self_Switch = function (
    args
  ) {
    this.pluginCommandBook_セルフスイッチの初期化(args);
  };

  Game_Interpreter.prototype.pluginCommandBook_セルフスイッチの遠隔操作 =
    function (args) {
  ...
  Game_Interpreter.prototype.pluginCommandBook_Remote_Control_Self_Switch =
    function (args) {
      this.pluginCommandBook_セルフスイッチの遠隔操作(args);
    };

  Game_Interpreter.prototype.pluginCommandBook_ピクチャの読み込み = function (
    args
  ) {
  ...
  Game_Interpreter.prototype.pluginCommandBook_戦闘アニメの読み込み = function (
    args
  ) {
    var hue = parseInt(args[1], 10).clamp(0, 360);
    ImageManager.loadAnimation(args[0], hue);
  };
  Game_Interpreter.prototype.pluginCommandBook_Load_Animation = function (
    args
  ) {
    this.pluginCommandBook_戦闘アニメの読み込み(args);
  };

  Game_Interpreter.prototype.pluginCommandBook_シャットダウン = function (
    args
  ) {
  ...
  Game_Interpreter.prototype.pluginCommandBook_ウェブサイト起動 = function (
    args
  ) {
    if (Utils.isNwjs()) {
  ...
  Game_Interpreter.prototype.pluginCommandBook_Startup_Website = function (
    args
  ) {
  ...
  Game_Interpreter.prototype.pluginCommandBook_ControlVariable = function (
    args
  ) {
  ...
  Game_Interpreter.prototype.pluginCommandBook_タッチ座標の取得 = function (
    args
  ) {
    var x, y;
  ...
  Game_Interpreter.prototype.pluginCommandBook_Get_Touch_Info = function (
    args
  ) {
    this.pluginCommandBook_タッチ座標の取得(args);
  };

  Game_Interpreter.prototype.pluginCommandBook_マップタッチ禁止の変更 =
    function (args) {
      $gameSystem._mapTouchDisable =
        args[0] === "禁止" || args[0] === "Disable";
    };
  Game_Interpreter.prototype.pluginCommandBook_Change_Map_Touch = function (
    args
  ) {
    this.pluginCommandBook_マップタッチ禁止の変更(args);
  };

  Game_Interpreter.prototype.pluginCommandBook_マップタッチ移動中判定 =
    function (args) {
      $gameSwitches.setValue(
        parseIntStrict(args[0]),
        $gameTemp.isDestinationValid()
      );
    };
  Game_Interpreter.prototype.pluginCommandBook_Get_Map_Touch_Moving = function (
    args
  ) {
    this.pluginCommandBook_マップタッチ移動中判定(args);
  };

  Game_Interpreter.prototype.pluginCommandBook_マップタッチ移動 = function (
    args
  ) {
    $gameTemp.setDestination(parseInt(args[0], 10), parseInt(args[1], 10));
  };

  Game_Interpreter.prototype.pluginCommandBook_Map_Touch_Move = function (
    args
  ) {
    this.pluginCommandBook_マップタッチ移動(args);
  };

  Game_Interpreter.prototype.pluginCommandBook_ピクチャの移動 = function (
    args
  ) {
  ...
  Game_Interpreter.prototype.pluginCommandBook_数値入力範囲の設定 = function (
    args
  ) {
  ...
  Game_Interpreter.prototype.pluginCommandBook_Set_Input_Num_Range = function (
    args
  ) {
    this.pluginCommandBook_数値入力範囲の設定(args);
  };
  Game_Interpreter.prototype.pluginCommandBook_数値入力ウィンドウの設定 =
    function (args) {
      var background;
  ...
  Game_Interpreter.prototype.pluginCommandBook_Set_Num_Input_Window = function (
    args
  ) {
    this.pluginCommandBook_数値入力ウィンドウの設定(args);
  };

  Game_Interpreter.prototype.pluginCommandBook_数値入力有効桁の設定 = function (
    args
  ) {
    $gameMessage._numInputValidDigit = parseIntStrict(args[0]).clamp(1, 8);
  };
  Game_Interpreter.prototype.pluginCommandBook_Set_Num_Input_Valid_Digit =
    function (args) {
      this.pluginCommandBook_数値入力有効桁の設定(args);
    };

  Game_Interpreter.prototype.pluginCommandBook_ピクチャの有効判定 = function (
    args
  ) {
    var picture = $gameScreen.picture(
      $gameScreen.realPictureId(parseIntStrict(args[1]))
    );
    $gameSwitches.setValue(parseIntStrict(args[0]), picture != null);
  };
  Game_Interpreter.prototype.pluginCommandBook_Get_Picture_Valid = function (
    args
  ) {
    this.pluginCommandBook_ピクチャの有効判定(args);
  };

  Game_Interpreter.prototype.pluginCommandBook_ピクチャの表示優先度設定 =
    function (args) {
      var picture = $gameScreen.picture(
        $gameScreen.realPictureId(parseIntStrict(args[0]))
      );
      if (picture) {
        picture.setZ(parseIntStrict(args[1]));
      } else {
        throw new Error(
          "指定された番号[" + args[0] + "]のピクチャは無効です。"
        );
      }
    };
  Game_Interpreter.prototype.pluginCommandBook_Set_Picture_Priority = function (
    args
  ) {
    this.pluginCommandBook_ピクチャの表示優先度設定(args);
  };

  Game_Interpreter.prototype.pluginCommandBook_ピクチャのトリミング = function (
    args
  ) {
    var picture = $gameScreen.picture(
      $gameScreen.realPictureId(parseIntStrict(args[0]))
    ...
    if (picture) {
      picture.setFrameDirect(
        parseIntStrict(args[1]),
        parseIntStrict(args[2]),
        parseIntStrict(args[3]),
        parseIntStrict(args[4])
      );
    } else {
      throw new Error("指定された番号[" + args[0] + "]のピクチャは無効です。");
    }
  };
  Game_Interpreter.prototype.pluginCommandBook_Trimming_Picture = function (
    args
  ) {
    this.pluginCommandBook_ピクチャのトリミング(args);
  };

  Game_Interpreter.prototype.pluginCommandBook_ピクチャの回転角設定 = function (
    args
  ) {
    var picture = $gameScreen.picture(
      $gameScreen.realPictureId(parseIntStrict(args[0]))
  ...
  Game_Interpreter.prototype.pluginCommandBook_ConcatenStrings = function (
    args
  ) {
  ...
  Game_Interpreter.prototype.pluginCommandBook_ReplaceStrings = function (
    args
  ) {
  ...
  Game_Interpreter.prototype.pluginCommandBook_文字一致カウント = function (
    args
  ) {
    args[0] = args[0].replace("#", "");
    var VarId1 = parseIntStrict(args[0]);
  ...
  Game_Interpreter.prototype.pluginCommandBook_CountMatchString = function (
    args
  ) {
  ...
  Game_Interpreter.prototype.pluginCommandBook_文字数カウント = function (
    args
  ) {
    args[0] = args[0].replace("#", "");
    var VarId1 = parseIntStrict(args[0]);
  ...
  Game_Interpreter.prototype.pluginCommandBook_CountStringlength = function (
    args
  ) {
  ...
  Game_Interpreter.prototype.pluginCommandBook_指定位置にアニメーション表示 =
    function (args) {
      ...
      var wait =
        args[3] &&
        (args[3] === "ウェイトあり" || args[3].toUpperCase() === "wait");
      $gameScreen.startAnimation(x, y, id, false);
      if (wait) this.wait($dataAnimations[id].frames.length * 4);
    };

  Game_Interpreter.prototype.pluginCommandBook_Show_Animation = function (
    args
  ) {
    this.pluginCommandBook_指定位置にアニメーション表示(args);
  };

  Game_Interpreter.prototype.pluginCommandBook_指定位置にループアニメーション表示 =
    function (args) {
      var x = parseIntStrict(args[0]);
      var y = parseIntStrict(args[1]);
      var id = parseIntStrict(args[2]);
  ...
  Game_Interpreter.prototype.pluginCommandBook_Show_Loop_Animation = function (
    args
  ) {
    this.pluginCommandBook_指定位置にループアニメーション表示(args);
  };

  Game_Interpreter.prototype.pluginCommandBook_ループアニメーション消去 =
    function (args) {
      $gameScreen.animationLoop = false;
    };

  Game_Interpreter.prototype.pluginCommandBook_Erase_Loop_Animation = function (
    args
  ) {
  ...
  if (eval(String(parameters["制御文字の拡張"] || "false"))) {
    (function () {
      var Window_Base_convertEscapeCharacters =
  ...
  if (eval(String(parameters["スクリプトに制御文字適用"] || "false"))) {
    (function () {
      ...
      Game_Interpreter.prototype.command355 = function () {
        ...
        this._list[index].parameters[0] = this.pluginCommandBook_unescape(
          this._list[index].parameters[0]
        );
        while (this._list[index + ++i].code === 655) {
          oldParams.push(this._list[index + i].parameters[0]);
          this._list[index + i].parameters[0] = this.pluginCommandBook_unescape(
            this._list[index + i].parameters[0]
          );
        }
        try {
          ...
          console.log(
            "- スクリプト   : " + this.currentCommand().parameters[0]
          );
          console.log("- エラー原因   : " + e.toString());
          result = true;
        }
      ...
      Game_Character.prototype.processMoveCommand = function (command) {
        if (command.code === Game_Character.ROUTE_SCRIPT) {
          ...
          command.parameters[0] =
            Game_Interpreter.prototype.pluginCommandBook_unescape(
              command.parameters[0]
            );
          try {
  ...
  Game_Picture.prototype.isValidFrame = function () {
    return (
      this._frameX + this._frameY + this._frameWidth + this._frameHeight > 0
    );
  };
  ...
  Scene_Map.prototype.isMapTouchOk = function () {
    return (
      (!$gameSystem._mapTouchDisable || $gameTemp.isDestinationValid()) &&
  ...
  Window_NumberInput.prototype.refresh = function () {
    if (this._number != null)
      this._number = this._number.clamp(
        $gameMessage._numInputMinValue,
        $gameMessage._numInputMaxValue
      );
  ...
  var _Window_NumberInput_changeDigit =
    Window_NumberInput.prototype.changeDigit;
  ...
  Spriteset_Base.prototype.updateAnimationContainer = function () {
    ...
    if (
      !this._animationContainer.isAnimationPlaying() &&
      this._animationId > 0
    ) {
      if ($gameScreen.animationLoop) {
        this._animationContainer.startAnimation(
  ...
  Spriteset_Base.prototype.sortPictures = function () {
    this._pictureContainer.children.sort(
      this._comparePictureChildOrder.bind(this)
    );
  };
