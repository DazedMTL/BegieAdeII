 ...
 *
 * @param acpCostHeader
 * @desc 消費アメリアCPの頭につける文字列
 * 初期値: Acp
 * @default Acp
 *
 * @param lcpCostHeader
 * @desc 消費リリックCPの頭につける文字列
 * 初期値: Lcp
 * @default Lcp
 *
 * @param hpCostColor
...
(function () {
  ...
  Window_SkillList.prototype.drawSkillCost = function (skill, x, y, width) {
    ...
    if (Imported.TMSkillCostEx) {
      ...
      cost = this._actor.skillLcpCost(skill);
      if (cost > 0) {
        x = this.drawSkillLcpCost(cost, x, y, count);
        if (count++ === maxCostNum - 1) {
          return;
        }
      }
      cost = this._actor.skillAcpCost(skill);
      if (cost > 0) {
        x = this.drawSkillAcpCost(cost, x, y, count);
        if (count++ === maxCostNum - 1) {
          return;
        }
      }
  ...
  Window_SkillList.prototype.drawSkillAcpCost = function (
    ...
    this.changeTextColor(this.textColor(acpCostColor));
    x -= this.textWidth(acpCost);
    this.drawText(acpCost, x, y);
  ...
  Window_SkillList.prototype.drawSkillLcpCost = function (
    ...
    this.changeTextColor(this.textColor(lcpCostColor));
    x -= this.textWidth(lcpCost);
    this.drawText(lcpCost, x, y);
