...
Moghunter.scrollBarAlwaysVisible = String(
  Moghunter.parameters["Always Visible"] || "true"
);
Moghunter.scrollBarX = Number(Moghunter.parameters["X-Axis"] || 0);
Moghunter.scrollBarY = Number(Moghunter.parameters["Y-Axis"] || 0);
Moghunter.scrollBar_ButtonX = Number(
  Moghunter.parameters["Button X-Axis"] || 1
);
Moghunter.scrollBar_ButtonY = Number(
  Moghunter.parameters["Button Y-Axis"] || 0
);
...
SpriteScrollBar.prototype.initialize = function (win) {
  ...
  this.visible = false;
  this._avisble =
    String(Moghunter.scrollBarAlwaysVisible) === "true" ? true : false;
  this.loadBitmaps();
  this.createSprites();
};
...
SpriteScrollBar.prototype.refreshButton = function () {
  ...
  for (var i = 0; i < this._button.length; i++) {
    ...
    if (i === 0) {
      p = [bx, by - this._img2.height];
    }
    if (i === 1) {
      p = [bx, by];
      this._button[i].setFrame(w, 0, w, h);
    }
    if (i === 2) {
      p = [bx, by + this.buttonSize() + this._img2.height];
    }
...
SpriteScrollBar.prototype.buttonPos = function () {
  ...
  var y =
    this.scrollBarY() +
    s * Math.floor(this._window.topIndex() / this._window.maxCols());
...
SpriteScrollBar.prototype.refreshBack = function () {
  ...
  for (var i = 0; i < this._back.length; i++) {
    ...
    if (i === 0) {
      p = [0, -this._img1.height];
    }
    if (i === 1) {
      p = [0, 0];
      this._back[i].setFrame(w, 0, w, h);
    }
    if (i === 2) {
      p = [0, this.heightF() + this._img1.height];
    }
    this._back[i].x = p[0];
    this._back[i].y = p[1];
...
SpriteScrollBar.prototype.needRefresh = function () {
  if (this._window._refScroll) {
    return true;
  }
  if (this._button._maxTopRow != this._window.maxTopRow()) {
    return true;
  }
  return false;
};
...
SpriteScrollBar.prototype.isVisible = function () {
  if (this._window._opening) {
    return false;
  }
  if (this._window._closing) {
    return false;
  }
  if (this._window.openness <= 0) {
    return false;
  }
  if (!this._window.visible) {
    return false;
  }
  if (this._window.maxTopRow() === 0) {
    return false;
  }
  if (!this._avisble && !this._window.active) {
    return false;
  }
  return true;
};
