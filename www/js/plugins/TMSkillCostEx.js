 ...
 * @type variable
 * @desc ダメージ計算とコスト消費の直前のタイミングで、
 * この変数に『アメリアが消費するCP』を代入する
 * @default 0
 *
 * @param lcpCostVNId
 * @type variable
 * @desc ダメージ計算とコスト消費の直前のタイミングで、
 * この変数に『リリックが消費するCP』を代入する
 * @default 0
 *
 ...
 *   <acpCost:10>         # スキルのコストとしてアメリアのＣＰ１０を設定
 *   <lcpCost:10>         # スキルのコストとしてリリックのＣＰ１０を設定
...
(function() {
	...
	var AcpCostVNId = +(parameters['acpCostVNId'] || 0);
	var LcpCostVNId = +(parameters['lcpCostVNId'] || 0);
	...
	Game_BattlerBase.prototype.skillAcpCost = function(skill) {
		return !this.isEnemy() && skill.meta.acpCost ? +skill.meta.acpCost : 0;
	};

	Game_BattlerBase.prototype.skillLcpCost = function(skill) {
		return !this.isEnemy() && skill.meta.lcpCost ? +skill.meta.lcpCost : 0;
	};
	...
	Game_BattlerBase.prototype.canPaySkillCost = function(skill) {
		if (!this.canPaySkillHpCost(skill) ||
				...
				!this.canPaySkillAcpCost(skill) ||
				!this.canPaySkillLcpCost(skill) ||
	...
	Game_BattlerBase.prototype.canPaySkillAcpCost = function(skill) {
		return $gameVariables.value(2) >= this.skillAcpCost(skill) || $gameVariables.value(2) === -1;
	};

	Game_BattlerBase.prototype.canPaySkillLcpCost = function(skill) {
		return $gameVariables.value(3) >= this.skillLcpCost(skill) || $gameVariables.value(3) === -1;
	};
	...
	Game_BattlerBase.prototype.paySkillCost = function(skill) {
		...
		if (HpCostVNId > 0) {
			$gameVariables.setValue(HpCostVNId, this.skillHpCost(skill));
		}
		if (MpCostVNId > 0) {
			$gameVariables.setValue(MpCostVNId, this.skillMpCost(skill));
		}
		if (TpCostVNId > 0) {
			$gameVariables.setValue(TpCostVNId, this.skillTpCost(skill));
		}
		if (AcpCostVNId > 0) {
			$gameVariables.setValue(AcpCostVNId, this.skillAcpCost(skill));
		}
		if (LcpCostVNId > 0) {
			$gameVariables.setValue(LcpCostVNId, this.skillLcpCost(skill));
		}
		this.paySkillHpCost(skill);
		this.paySkillItemCost(skill);
		this.paySkillExpCost(skill);
		this.paySkillGoldCost(skill);
		this.paySkillAcpCost(skill);
		this.paySkillLcpCost(skill);
	...
	Game_BattlerBase.prototype.paySkillAcpCost = function(skill) {
		$gameVariables.setValue(2, $gameVariables.value(2) - this.skillAcpCost(skill));
	};

	Game_BattlerBase.prototype.paySkillLcpCost = function(skill) {
		$gameVariables.setValue(3, $gameVariables.value(3) - this.skillLcpCost(skill));
	};
