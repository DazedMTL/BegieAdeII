...
Moghunter.bcom_zoom_effect = String(
  Moghunter.parameters["Zoom Animation"] || true
);
...
Moghunter.bcom_slide_effect = String(
...
Moghunter.bcom_com_name_x = Number(
  Moghunter.parameters["Com Name X-axis"] || 55
);
Moghunter.bcom_com_name_y = Number(
  Moghunter.parameters["Com Name Y-axis"] || 75
);
Moghunter.bcom_com_font_size = Number(
  Moghunter.parameters["Com Font Size"] || 22
);
...
Moghunter.bcom_cursor_slide = String(
  Moghunter.parameters["Cursor Slide"] || true
);
...
if (Imported.MOG_BattleHud) {
  ...
  Window_ActorCommand.prototype.com_pic_initialize = function () {
    ...
    if (
      String(Moghunter.bcom_slide_effect) === "true" &&
      this._com_mode_b != 1
    ) {
    ...
    for (var i = 0; i < this._list.length; i++) {
      ...
      for (var r = 0; r < this._list.length; r++) {
        this._com_images.push(
          ImageManager.loadBcom("Com_" + this._list[r].name)
        );
      }
    }
  ...
  Window_ActorCommand.prototype.update_cursor_b = function () {
    ...
    var nx =
      this._cursor_nxy[0] + Moghunter.bcom_cursor_x + this._cursor_b_s[2];
    var ny = this._cursor_nxy[1] + Moghunter.bcom_cursor_y;
    this._cursor_b.x = this.sprite_move_to(
      this._cursor_b.x,
      nx,
      this.com_move_speed()
    );
    this._cursor_b.y = this.sprite_move_to(
      this._cursor_b.y,
      ny,
      this.com_move_speed()
    );
  ...
  Window_ActorCommand.prototype.refresh_com_name = function () {
    ...
    this._com_name.bitmap.drawText(
      this.command_name(),
      0,
      0,
      this._com_name.bitmap.width,
      32,
      "center"
    );
  };
  ...
  Window_ActorCommand.prototype.refresh_bitmap_com = function () {
    ...
    for (var i = 0; i < this.maxCom(); i++) {
      ...
      this._com_sprites[i].y =
        this._wh[1] + this._wh[1] * i + Moghunter.bcom_com_y;
    }
  };
  ...
  Window_ActorCommand.prototype.refresh_arrow_position = function () {
    this._arrow_org_y = [
      -Moghunter.bcom_arrow_y,
      this._wh[1] * (this.limit_rows_sp() + 2) + Moghunter.bcom_arrow_y,
    ];
  ...
  // * Sprite Move To
  //==============================
  Window_ActorCommand.prototype.sprite_move_to = function (
    value,
    real_value,
    speed
  ) {
  ...
  Window_ActorCommand.prototype.update_position = function (i) {
    if (this._com_mode_b === 0) {
      ...
      this._np[1] =
        ...
        this._wh[1] * i -
        this.limit_height() +
        Moghunter.bcom_com_y +
        this._side[i][1];
    } else {
      this.update_ring_command(i, this._index);
      this._np[0] =
        this.width / 2 + this._wh[0] + Moghunter.bcom_com_x + this._side[i][0];
      this._np[1] =
        this.height / 2 + this._wh[1] + Moghunter.bcom_com_y + this._side[i][1];
    }
    this._com_sprites[i].x = this.sprite_move_to(
      this._com_sprites[i].x,
      this._np[0],
      this.com_move_speed()
    );
    this._com_sprites[i].y = this.sprite_move_to(
      this._com_sprites[i].y,
      this._np[1],
      this.com_move_speed()
    );
  ...
  Window_ActorCommand.prototype.update_on_index = function (i) {
    ...
    this._cursor_nxy = [
      this._com_sprites[i].x - this._com_sprites[i].bitmap.width / 2,
      this._com_sprites[i].y,
    ];
  ...
  Window_ActorCommand.prototype.update_zoom_effect = function (i) {
    if (this._zoom[0] === 0) {
      ...
      if (
        this._loop_zoom_effect &&
        this._com_sprites[i].scale.x >= this.com_zoom_rate()
      ) {
        this._zoom[0] = 1;
      }
    } else {
      ...
      if (this._com_sprites[i].visible && this._com_sprites[i].opacity > 25) {
        if (
          TouchInput.x >
            this.x +
              this._com_sprites[i].x -
              this._com_sprites[i].bitmap.width / 2 &&
          TouchInput.x <
            this.x +
              this._com_sprites[i].x +
              this._com_sprites[i].bitmap.width -
              this._com_sprites[i].bitmap.width / 2 &&
          TouchInput.y >
            this.y +
              this._com_sprites[i].y -
              this._com_sprites[i].bitmap.height / 2 &&
          TouchInput.y <
            this.y +
              this._com_sprites[i].y +
              this._com_sprites[i].bitmap.height -
              this._com_sprites[i].bitmap.height / 2
        ) {
          this._index = i;
          return true;
  ...
  //==============================
  var _alias_mog_bcom_sbat_create_layout_window =
    Scene_Battle.prototype.create_layout_window;
