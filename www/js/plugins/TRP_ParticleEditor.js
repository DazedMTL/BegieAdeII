...
(function () {
  ...
  var displayCount =
    PluginManager.parameters("TRP_Particle").displayCount === "true";
  ...
  (function () {
    ParticleEditor.GUIDE_TEXTS = {
      alpha: [
        ...
        null,
        "<中間値設定可能>",
        "左端が開始時(時間0)、右端が終了時(時間1)の値",
        "@時間(0~1) + Enterで中間値を設定可能",
        "delete(Macはfn+delete)で中間値の削除",
        "中間値編集中にshift+@時間(0~1)+Enterで時間変更",
        "例)@0.5 + Enterで中間地点での値を設定開始",
      ],
      scale: [
        "【スケール/拡大率】",
        "1で等倍",
      ...
      minimumScaleMultiplier: [
        "【最小スケール倍率】",
        "パーティクル生成時の最小スケール倍率",
        "例)0.1として開始時のスケールが0.5の場合は",
        "　 0.5*0.1=0.05から0.5の間の",
        "　 ランダムなスケールのパーティクルが発生。",
      ],
      speed: [
        "【スピード/速度】",
        "パーティクルの速度。開始値は0にしないこと。",
        null,
        "<中間値設定可能>",
        "左端が開始時(時間0)、右端が終了時(時間1)の値",
        "@時間(0~1) + Enterで中間値を設定可能",
      ...
      colorMode: [
        "【カラーモード】",
        "0で時間によって色が変化。",
        "0以外で発生時の色の分布を変化(時間変化無し)",
      ],
      acceleration: [
        "【加速度】",
        "加速度が有効な場合は画像の向きは移動方向で固定",
      ],
      startRotation: [
        "【発生時の射出角度】",
        "パーティクルの飛び出す方向。(画像の向きも連動)",
        "0で右,90で下,180で左,270(-90)で上方向。",
      ],
      rotationSpeed: [
        "【画像回転速度】",
        "画像の回転速度の最小値と最大値。",
      ...
      imageOption: [
        "【画像表示設定】",
        "画像の向き/反転の設定。動きには影響なし。",
        "angle:0~360で画像のみ回転、-1でランダムな角度。",
        "mirror:0で調整なし,1で反転,2でランダム。",
      ],
      lifetime: [
        "【パーティクル寿命】",
        "個々のパーティクルの消えるまでの秒数",
      ...
      frequency: [
        "【フリクエンシー/発生間隔】",
        "パーティクルが発生する間隔",
        "0.01とすると0.01秒に1回発生し、1秒間に100回",
      ],
      spawnChance: [
        "【発生確率】",
        "各発生タイミングに、パーティクルが発生する確率。",
      ],
      particlesPerWave: [
        "【1回での発生個数】",
        "各発生タイミングに同時に発生する個数。",
        "同時に発生するパーティクルのlifetimeは固定",
      ],
      emitterLifetime: [
        "【エミッター寿命】",
        "-1で無限。0より大きくすると、指定時間で発生終了",
      ],
      maxParticles: [
        "【最大表示数の制限】",
        "このエミッターから発生する最大数の制限値",
      ],
      fluctuation: [
        "【フラクチュエーション/ゆらぎ】",
        "角度のゆらぎ設定。",
        "max:各フレームで角度の変化幅の最大値",
        "sensitivity:0~1で指定。小さいほどゆるやかに、大きいほど激しくゆらぐ。",
      ],
      pos: [
        "【ポイント(発生方法)】",
        "指定の点から発生。",
      ...
      rect: [
        "【レクト/四角(発生方法)】",
        "四角形の中からランダムに発生。",
        "左上の点をx,y、幅/高さをwidth,heightで指定",
        "<※発生方法はいずれか１つのみ指定が有効>",
      ],
      circle: [
        "【サークル/円(発生方法)】",
        "円の中からからランダムに発生",
        "中心点をx,y、半径をradiusで指定。",
        "<※発生方法はいずれか１つのみ指定が有効>",
      ],
      ring: [
        "【リング(発生方法)】",
        "ドーナツ状のリングからランダムに発生。",
        "中心点をx,y、外側の半径をradius",
        "内側の半径をminRで指定。",
        "<※発生方法はいずれか１つのみ指定が有効>",
      ],
      burst: [
        "【バースト(発生方法)】",
        "particlesPerWaveで複数発生させる際に",
        "開始角度startAngleと角度間隔spacingを指定して",
        "放射状に発生させる。(startRotation設定無効)",
        "particlesPerWaveとspacingをかけて360とすると、",
        "きれいな放射状に発生する",
        "<※発生方法はいずれか１つのみ指定が有効>",
      ],
    };
  ...
  ParticleEmitter.prototype.updateEmitter = function () {
    ...
    if (
      ...
      this._data.editing &&
      this._emitter &&
      (this._emitter._emitterLife === 0 ||
        !this._emitter.emit ||
        this.isEmitByOriginalTrigger()) &&
  ...
  var _ParticleEmitter_updateForStartDash =
    ParticleEmitter.prototype.updateForStartDash;
...
===================================*/
  ...
  ParticleEditor.prototype.processInput = function () {
    if (Input.isRepeated("down") && this._keyCode !== 98) {
      ...
      this._keyCode >= KEY_CODE.alphabet &&
      this._keyCode <= KEY_CODE.alphabetEnd
    ) {
      this.pushInputtingCharacter(String.fromCharCode(this._keyCode));
    } else {
      var editing =
        this._editingIndex >= 0 ? this._parts[this._editingIndex] : null;
      if (editing) {
...
===================================*/
  ...
  ParticleEditor.prototype.addValueParts = function (
    y,
    data,
    config,
    configNames,
    ...
    var parts = new ParticleParam.ValueParam(
      data,
      config,
      configNames,
      title,
  ...
  ParticleEditor.prototype.addNodeParts = function (
    y,
    data,
    config,
    configNames,
  ...
  ParticleEditor.prototype.addColorNodeParts = function (
    y,
    data,
    config,
    configNames,
    ...
    var parts = new ParticleParam.ColorNodeParam(
      data,
      config,
      configNames,
      title
...
===================================*/
  ...
  ParticleEditor.prototype.menuButtonSprite = function (
    index,
    xOffset,
    color,
    name,
    ...
    sprite.x =
      Graphics.boxWidth - margin - index * (width + margin) - width - xOffset;
  ...
  ParticleEditor.prototype.checkImageExists = function (data) {
    ...
    var splitAnimImages =
      PluginManager.parameters("TRP_ParticleList").animImages || [];
...
===================================*/
  ParticleEditor.prototype.writeHelpFile = function () {
    ...
    var animImages =
      PluginManager.parameters("TRP_ParticleList").animImages || null;
...
===================================*/
  ...
  ParticleParam.prototype.initialize = function (
    data,
    config,
    configNames,
    title
...
===================================*/
  ...
  ParticleParam.prototype.processInput = function (keyCode) {
    if (Input.isTriggered("cancel") && keyCode !== 96) {
      ...
      (Input.isTriggered("right") && keyCode !== 102)
    ) {
  ...
  ParticleParam.prototype.value = function (config) {
    return 0;
  };
  ...
  ValueParam.prototype.initialize = function (
    data,
    config,
    name,
    title,
  ...
  NodeParam.prototype.partsText = function (index) {
    ...
    var text =
      editing && this._inputtingTime === null
        ? this._inputting
        : String(data.value);
...
===================================*/
  NodeParam.prototype.processInput = function (keyCode) {
    if (keyCode === KEY_CODE.delete) {
      if (
        ...
        this._editingIndex !== this._innerData.length - 1
      ) {
    ...
    } else if (isMac ? keyCode === KEY_CODE.bracket : keyCode === KEY_CODE.at) {
      //[ > new
      // mac > @ = bracket
      this.setEditing(-1);
      this._inputtingTime = "";
  ...
  NodeParam.prototype.processInputForInputtingTime = function (keyCode) {
    ...
    if (Input.isTriggered("ok")) {
      ...
      (keyCode >= numKeyCode && keyCode < numKeyCode + 10) ||
      (keyCode >= tenKeyCode && keyCode < tenKeyCode + 10)
    ) {
      var num =
        keyCode >= tenKeyCode ? keyCode - tenKeyCode : keyCode - numKeyCode;
      if (this._inputtingTime.contains(".")) {
...
===================================*/
  ...
  ColorNodeParam.prototype._partsText = function (index, noColorCode) {
    ...
    if (noColorCode) {
      ...
      text =
        index === this._editingIndex ? this._inputting : String(data.value);
    }
    if (index > 0 && index < this._innerData.length - 1) {
      // text += '@'+data.time;
      text += this._timeText(index);
  ...
  ColorNodeParam.prototype.colorCodeWithValue = function (value) {
    if (value.indexOf("rgb") === 0) {
      ...
      return (
        "#" +
        Number(values[1]).toString(16) +
        Number(values[2]).toString(16) +
        Number(values[3].toString(16))
      );
    } else {
      return value;
    }
  ...
  ColorPicker.prototype.initialize = function (size) {
    ...
    bitmap.clearRect(
      lineWidth,
      lineWidth,
      bitmap.width - lineWidth * 2,
      bitmap.height - lineWidth * 2
    );
  ...
  ColorPicker.prototype.updateResultColor = function () {
    this._color = ColorPicker.colorWithHsv(
      this._hue,
      this._saturation,
      this._value
    );
  ...
  ColorPicker.prototype.update = function () {
    ...
    touchInside =
      dx >= 0 && dx <= hPicker.width && dy >= 0 && dy <= hPicker.height;
    ...
    dy = y - svPicker.y;
    touchInside =
      dx >= 0 && dx <= svPicker.width && dy >= 0 && dy <= svPicker.height;
    if (this._touchingSv || (!this._touchingHue && touchInside)) {
      ...
      var v = Math.round(
        ((svPicker.height - 1 - dy) / (svPicker.height - 1)) * 255
      );
      this.setSV(s, v);
  ...
  PickerBase.prototype._refresh = function () {
    ...
    this._dispRows = Math.floor(
      (Graphics.boxHeight - 2 * margin) / (itemHeight + my)
    );
...
===================================*/
  ...
  PickerBase.prototype.itemHeight = function () {
    return 48;
  };
  PickerBase.prototype.itemWidth = function () {
    return 48;
  };
  PickerBase.prototype.maxItems = function () {
    return 0;
  };
  PickerBase.prototype.guideTexts = function () {
    return null;
  };
  PickerBase.prototype.itemMarginX = function () {
    return 0;
  };
  PickerBase.prototype.itemMarginY = function () {
    return 0;
  };
  PickerBase.prototype.categoryType = function () {
    return 1;
  };
  PickerBase.prototype.headerText = function () {
    return "";
  };

  //category
  PickerBase.prototype.maxCategories = function () {
    return 1;
  };
  PickerBase.prototype.isCategoryValid = function (index) {
    return true;
  };
  PickerBase.prototype.isSeveralModeValid = function () {
    return true;
  };
...
===================================*/
  ...
  PickerBase.prototype.setSelectingIndex = function (index) {
    ...
    sprite.tint = this._severalMode
      ? PickerBase.TINT_SEVERAL
      : PickerBase.TINT_NORMAL;
...
===================================*/
  ...
  ImagePickerBase.prototype.imageName = function () {
    return "";
  };
...
===================================*/
  ImagePicker.prototype.refreshItems = function () {
    ...
    for (var i = 0; i < length; i = (i + 1) | 0) {
      ...
      bitmap.addLoadListener(
        this._adjustSpriteScale.bind(this, sprite, itemWidth, itemHeight)
      );
  ...
  ImagePicker.prototype._adjustSpriteScale = function (
    sprite,
    width,
    height,
    bitmap
...
===================================*/
  ...
  TilePicker.prototype.imageName = function () {
    ...
    var bitmapName = bitmapNameElems[bitmapNameElems.length - 1].replace(
      ".png",
      ""
    );
    var name = "";
...
===================================*/
  ...
  PresetPicker.prototype.isSeveralModeValid = function () {
    return false;
  };
