 ...
 * @default 3
 *
 * @param Party TPNumber X-Axis
 * @desc Definição X-Axis do número de TP.
 * @default 118
 *
 * @param Party TPNumber Y-Axis
 ...
 * @default 30
 *
 * @param Party TPMeter X-Axis
 * @desc Definição X-Axis do medidor de TP.
 * @default 113
 *
 * @param Party TPMeter Y-Axis
...
Moghunter.scItem_ItemWindowWidth = Number(
  Moghunter.parameters["Item Width"] || 370
);
Moghunter.scItem_ItemWindowHeight = Number(
  Moghunter.parameters["Item Height"] || 400
);
Moghunter.scItem_ItemWindowX = Number(Moghunter.parameters["Item X-Axis"] || 0);
Moghunter.scItem_ItemWindowY = Number(
  Moghunter.parameters["Item Y-Axis"] || 96
);
Moghunter.scItem_HelpWindowX = Number(Moghunter.parameters["Help X-Axis"] || 0);
Moghunter.scItem_HelpWindowY = Number(
  Moghunter.parameters["Help Y-Axis"] || 516
);
Moghunter.scItem_WlayoutX = Number(
  Moghunter.parameters["W Layout X-Axis"] || 0
);
Moghunter.scItem_WlayoutY = Number(
  Moghunter.parameters["W Layout Y-Axis"] || 0
);
Moghunter.scItem_Com1_X = Number(
  Moghunter.parameters["Com Item X-Axis"] || 280
);
Moghunter.scItem_Com1_Y = Number(Moghunter.parameters["Com Item Y-Axis"] || 50);
Moghunter.scItem_Com2_X = Number(
  Moghunter.parameters["Com Weapon X-Axis"] || 420
);
Moghunter.scItem_Com2_Y = Number(
  Moghunter.parameters["Com Weapon Y-Axis"] || 50
);
Moghunter.scItem_Com3_X = Number(
  Moghunter.parameters["Com Armour X-Axis"] || 560
);
Moghunter.scItem_Com3_Y = Number(
  Moghunter.parameters["Com Armour Y-Axis"] || 50
);
...
Moghunter.scItem_ActorWinX = Number(
  Moghunter.parameters["Actor Windows X-Axis"] || 600
);
Moghunter.scItem_ActorWinY = Number(
  Moghunter.parameters["Actor Windows Y-Axis"] || 200
);
Moghunter.scItem_ActorFaceX = Number(
  Moghunter.parameters["Actor Face X-Axis"] || 90
);
Moghunter.scItem_ActorFaceY = Number(
  Moghunter.parameters["Actor Face Y-Axis"] || -65
);
Moghunter.scItem_ActorStatesX = Number(
  Moghunter.parameters["Actor States X-Axis"] || 23
);
Moghunter.scItem_ActorStatesY = Number(
  Moghunter.parameters["Actor States Y-Axis"] || 15
);
Moghunter.scItem_ActorParX = Number(
  Moghunter.parameters["Actor Parameters X-Axis"] || 0
);
Moghunter.scItem_ActorParY = Number(
  Moghunter.parameters["Actor Parameters Y-Axis"] || 0
);
Moghunter.scItem_ActorParFontSize = Number(
  Moghunter.parameters["Actor Parameters FontSize"] || 20
);
Moghunter.scItem_PartyX = Number(
  Moghunter.parameters["Party Window X-Axis"] || 10
);
Moghunter.scItem_PartyY = Number(
  Moghunter.parameters["Party Window Y-Axis"] || 240
);
Moghunter.scItem_PartyFontSize = Number(
  Moghunter.parameters["Party Font Size"] || 20
);
Moghunter.scItem_PartyNameX = Number(
  Moghunter.parameters["Party Name X-Axis"] || 20
);
Moghunter.scItem_PartyNameY = Number(
  Moghunter.parameters["Party Name Y-Axis"] || 40
);
Moghunter.scItem_PartyHPNumberX = Number(
  Moghunter.parameters["Party HPNumber X-Axis"] || 20
);
Moghunter.scItem_PartyHPNumberY = Number(
  Moghunter.parameters["Party HPNumber Y-Axis"] || 3
);
Moghunter.scItem_PartyMPNumberX = Number(
  Moghunter.parameters["Party MPNumber X-Axis"] || 118
);
Moghunter.scItem_PartyMPNumberY = Number(
  Moghunter.parameters["Party MPNumber Y-Axis"] || 3
);
Moghunter.scItem_PartyTPNumberX = Number(
  Moghunter.parameters["Party TPNumber X-Axis"] || 118
);
Moghunter.scItem_PartyTPNumberY = Number(
  Moghunter.parameters["Party TPNumber Y-Axis"] || 3
);
Moghunter.scItem_PartyHPMeterX = Number(
  Moghunter.parameters["Party HPMeter X-Axis"] || 16
);
Moghunter.scItem_PartyHPMeterY = Number(
  Moghunter.parameters["Party HPMeter Y-Axis"] || 30
);
Moghunter.scItem_PartyMPMeterX = Number(
  Moghunter.parameters["Party MPMeter X-Axis"] || 113
);
Moghunter.scItem_PartyMPMeterY = Number(
  Moghunter.parameters["Party MPMeter Y-Axis"] || 30
);
Moghunter.scItem_PartyTPMeterX = Number(
  Moghunter.parameters["Party TPMeter X-Axis"] || 113
);
Moghunter.scItem_PartyTPMeterY = Number(
  Moghunter.parameters["Party TPMeter Y-Axis"] || 30
);
Moghunter.scItem_PartyStatusX = Number(
  Moghunter.parameters["Party Status X-Axis"] || 165
);
Moghunter.scItem_PartyStatusY = Number(
  Moghunter.parameters["Party Status Y-Axis"] || 39
);
  ...
  for (var i = 0; i < this._buttons.length; i++) {
    if (this._categoryWindow._index === i && !this._actorWindow.active) {
      if (this._buttonsAni[i] === 0) {
        this._buttons[i].scale.x += 0.01;
        if (this._buttons[i].scale.x >= 1.3) {
          this._buttons[i].scale.x = 1.3;
          this._buttonsAni[i] = 1;
        }
      } else {
        this._buttons[i].scale.x -= 0.01;
        if (this._buttons[i].scale.x <= 1.0) {
          this._buttons[i].scale.x = 1.0;
          this._buttonsAni[i] = 0;
        }
      }
...
Scene_Item.prototype.addCatIndex = function (value) {
  ...
  if (this._categoryWindow._index > 3) {
    this._categoryWindow._index = 0;
  }
  if (this._categoryWindow._index < 0) {
    this._categoryWindow._index = 3;
  }
  if (this._wani[2] != null) {
    this._categoryWindow._index = this._wani[2];
  }
...
Scene_Item.prototype.checkTouchOnSprites = function () {
  if (this._itemWindow.active) {
    ...
    for (var i = 0; i < this._partyWindow.length; i++) {
      if (this.isOnSprite2(this._partyWindow[i])) {
        this.setTouchParty(i);
        touch = true;
      }
    }
    if (!this._actorWindow._cursorAll) {
      for (var i = 0; i < this._partyArrow.length; i++) {
        if (this.isOnSprite(this._partyArrow[i])) {
    ...
    if (!touch) {
      if (this.isOnSprite3(this._itemWindow)) {
...
Scene_Item.prototype.addIndexActorWindow = function (value) {
  ...
  if (this._actorWindow._index >= $gameParty.members().length) {
    this._actorWindow._index = 0;
  }
  if (this._actorWindow._index < 0) {
    this._actorWindow._index = $gameParty.members().length - 1;
  }
  SoundManager.playCursor();
};
...
Scene_Item.prototype.isOnSprite2 = function (sprite) {
  ...
  if (TouchInput.x < sprite.x) {
    return false;
  }
  if (TouchInput.x > sprite.x + cw) {
    return false;
  }
  if (TouchInput.y < sprite.y) {
    return false;
  }
  if (TouchInput.y > sprite.y + ch) {
    return false;
...
Scene_Item.prototype.updateActorStatus = function () {
  if (this._actorStatusIndex != this._actorWindow._index) {
...
Scene_Item.prototype.updateActorDataWindow = function () {
  ...
  this._actorDataWindow.x = nx;
  this._actorDataWindow.y = ny;
...
Scene_Item.prototype.createPartyData = function () {
  ...
  this._partyPos[2] = [
    Moghunter.scItem_PartyX - 20,
    Moghunter.scItem_PartyY - 100,
  ];
  this._partyPos[3] = [
    Moghunter.scItem_PartyX - 20,
    Moghunter.scItem_PartyY + 100,
...
Scene_Item.prototype.updatePartyWindow = function () {
  for (var i = 0; i < this._partyWindow.length; i++) {
    ...
    this._partyWindow[i].x = nx;
    this._partyWindow[i].y = ny;
  }
...
Scene_Item.prototype.updatePartyArrow = function () {
  for (var i = 0; i < this._partyArrow.length; i++) {
    ...
    this._partyArrow[i].x = nx;
    this._partyArrow[i].y = ny;
  }
};
...
Scene_Item.prototype.createPartyArrow = function () {
  ...
  for (var i = 0; i < 2; i++) {
    ...
    if ($gameParty.members().length < 4) {
      this._partyArrow[i].visible = false;
    }
    if (i === 1) {
      this._partyArrow[i].scale.y = -1.0;
    }
...
PartyWindowData.prototype.loadBitmaps = function () {
  ...
  this._tpMeterImg = ImageManager.loadMenusActor("TPMeter");
};
...
PartyWindowData.prototype.refreshPar = function () {
  ...
  this._par.bitmap.drawText(
    this._actor.hp,
    Moghunter.scItem_PartyHPNumberX,
    Moghunter.scItem_PartyHPNumberY,
    80,
    32,
    "right"
  );
  this._par.bitmap.drawText(
    this._actor.mp,
    Moghunter.scItem_PartyMPNumberX,
    Moghunter.scItem_PartyMPNumberY,
    80,
    32,
    "right"
  );
  this._par.bitmap.drawText(
    this._actor.tp,
    Moghunter.scItem_PartyTPNumberX,
    Moghunter.scItem_PartyTPNumberY,
    80,
    32,
    "right"
  );
...
// * createTP Meter
//==============================
PartyWindowData.prototype.createTPMeter = function () {
  this._tpMeter = new Sprite(this._tpMeterImg);
  this._tpMeter.x = Moghunter.scItem_PartyTPMeterX;
  this._tpMeter.y = Moghunter.scItem_PartyTPMeterY;
...
// * refreshTP Meter
//==============================
PartyWindowData.prototype.refreshTPMeter = function () {
  var cw = this._tpMeterImg.width;
  var ch = this._tpMeterImg.height;
  var wid = (cw * this._actor.tp) / 100;
  this._tpMeter.setFrame(0, 0, wid, cw);
};
...
PartyWindowData.prototype.createStates = function () {
  ...
  this._text = new Sprite(new Bitmap(100, 100));
  this._text.x = 236;
  this._text.y = 43;
  this._text.bitmap.fontSize = 17;
  this.addChild(this._text);
  this._text.bitmap.drawText($gameVariables.value(1), 0, 0, 50, 50, "right"); // inyoku-do
...
PartyWindowData.prototype.update = function () {
  ...
  if (!this._par && this._layImg.isReady()) {
    this.createSprites();
  }
  if (this._state_icon) {
    this.update_states();
...
ActorDataWindow.prototype.refreshFaces = function () {
  this._faces.bitmap = ImageManager.loadMenusFaces1(
    "Actor_" + this._actor._actorId
  );
};
