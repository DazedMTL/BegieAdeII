...
var _EBE_Game_Party_requestMotionRefresh =
  ...
  if (!this._interpreter.isRunning()) {
    ...
    for (var i = 0; i < eventIds.length; i++) {
      if (eventIds[i]) {
        ...
        if (
          this.meetsCommonEventCommentConditions(event) &&
          !this._commonEventFlags[i]
        ) {
...
Game_Troop.prototype.setupEbeBattleEvent = function (condition, metacodes) {
  if (!this._interpreter.isRunning()) {
    ...
    for (var i = 0; i < pages.length; i++) {
      var page = pages[i];
      if (
        this.meetsPagesCommentConditions(page, metacodes) &&
        !this._eventFlags[i]
      ) {
        ...
        switch ((match[1] + "").toUpperCase()) {
          ...
          case "ENEMY_HP":
            if (match[2] === ":") {
              ...
              if (!value)
                value = /#(\d+)[ ]*less[ ]than[ ]*(\d+)[ ]*\%/i.exec(match[3]);
              if (value) {
