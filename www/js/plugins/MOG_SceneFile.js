...
Moghunter.scFile_PlayTimeWord = String(
  Moghunter.parameters["Word Playtime"] || "Playtime"
);
Moghunter.scFile_LocationWord = String(
  Moghunter.parameters["Word Location"] || "Location"
);
...
Moghunter.scFile_ParBackY = Number(
  Moghunter.parameters["Parback Y-Axis"] || 320
);
...
Moghunter.scFile_CursorS = Number(
  Moghunter.parameters["Cursor File Space"] || 159
);
...
Scene_File.prototype.create = function () {
  ...
  this._Cursor.x =
    Moghunter.scFile_CursorX +
    Moghunter.scFile_CursorS * this._listWindow._index;
};
...
Scene_File.prototype.nextIndex = function (value) {
  ...
  if (this._listWindow._index < 0) {
    this._listWindow._index = 2;
  }
  if (this._listWindow._index > 2) {
    this._listWindow._index = 0;
  }
};
...
Scene_File.prototype.isOnFile = function (index) {
  ...
  if (TouchInput.x > x + cw) {
    return false;
  }
  if (TouchInput.y < y) {
    return false;
  }
  if (TouchInput.y > y + ch) {
    return false;
  }
  return true;
};
...
Scene_File.prototype.refreshActorPicture = function () {
  ...
  this._ActorPicture.bitmap = ImageManager.loadPicture(
    "Actor_" + String(this._actorIndex)
  );
...
Scene_File.prototype.updateActorPicture = function () {
  if (!this._ActorPicture.bitmap || !this._ActorPicture.bitmap.isReady()) {
    return;
  }
  ...
  this._ActorPicture.y =
    Graphics.boxHeight - this._ActorPicture.height + Moghunter.scFile_ActorY;
};
...
Window_FileData.prototype.drawData = function () {
  ...
  if (this._gold) {
    this.drawText(TextManager.currencyUnit, x, y + s * 9, 120, "left");
  } // Gold
  this.resetTextColor();
