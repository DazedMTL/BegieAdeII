...
!(function () {
  ...
  class Game_Spine {
    ...
    setAnimation(id, animations, ...args) {
      ...
      list.forEach((animation) => {
        let [name, options] = `${animation}/`.split(
          / *\/ *(?=(?:times|timeScale|alpha|$))/i
        );
        let times = 1;
        let timeScale = 1.0;
        let alpha = 1.0;
    ...
    setAlpha(id, value, overwrite = false) {
      ...
      this._alpha[id] = {
        value: Number(value),
        overwrite:
          typeof overwrite == "boolean" ? overwrite : overwrite == "true",
      };
      return this;
    }
    ...
    setBone(boneName, x, y, rotation, duration = 1, screen = true) {
      [boneName, x, y, rotation, duration, screen] = convars([
        boneName,
        x,
        y,
        rotation,
    ...
    updatePlayData(entry, reason) {
      ...
      switch (reason) {
        case "start":
          ...
          break;
        case "interrupt":
          index = playData.findIndex((data) =>
            ["play", "repeat"].includes(data.state)
          );
          state = "suspend";
          break;
        case "complete":
          index = playData.findIndex((data) =>
            ["play", "suspend"].includes(data.state)
          );
          if (index >= 0) {
  ...
  class Sprite_Spine extends Sprite {
    ...
    updateSkin(spine) {
      ...
      let Skin =
        (PIXI.spine.core || PIXI.spine).Skin ||
        this._data.spineData.skins[0].constructor;
      let skin = new Skin(" ");
      for (let name of this._skin) {
        if (name) {
          name =
            this._skinNames.find((skinName) => {
    ...
    updateColor(spine) {
      ...
      for (let image in spine.color) {
        if (spine.color[image] != this._color[image]) {
          ...
          let sprites =
            image == "/default/" ? [this._data] : this.getSpineSprites(image);
          ...
          if (image != "/default/") {
            let count =
              this.getAttachments(image, "MeshAttachment").length +
    ...
    updateBone(spine) {
      ...
      for (let name in this._bone) {
        ...
        if (bone_p.screen) {
          [bone_p.x, bone_p.y] = this.computeLocalVertices([
            bone_p.x,
            bone_p.y,
          ]);
        }
      ...
      for (let id in spine.track) {
        if (spine.track[id] != this._track[id]) {
          ...
          list.forEach((animation, index) => {
            let name =
              this._animationNames.find((animationName) => {
          ...
          switch (data.state) {
            ...
            case "done":
              ...
              while (entry.trackTime < data.trackTime) {
                this._data.state.timeScale = Math.min(
                  data.trackTime - entry.trackTime,
                  0.01
                );
                this._data.update(1);
              }
            ...
            case "play":
            case "repeat":
              data.trackTime %= this._data.spineData.animations.find(
                (animation) => animation.name == data.name
              ).duration;
            ...
            default:
              this._data.stateData.defaultMix = data.mixDuration;
              entry = this._data.state.addAnimation(
                id,
                data.name,
                data.loop,
                0
    ...
    getAttachmentData(slotName, screen = true) {
      ...
      if (attachmentType.match(/^PointAttachment/)) {
        ...
        if (screen) {
          [position.x, position.y] = this.computeScreenVertices([
            position.x,
            position.y,
          ]);
        }
    ...
    hitTest(x, y, screen = true) {
      ...
      for (let i = 0; i < polygons.length; i++) {
        ...
        if (bounds.containsPointPolygon(polygon, x, y)) {
          ...
          let slot = this._data.skeleton.slots.find(
    ...
    onEvent(entry, event) {
      ...
      if (audioPath) {
        ...
        let volume = volumeId
          ? $gameVariables.value(volumeId)
          : audioData.volume;
        let balance = balanceId
          ? $gameVariables.value(balanceId)
          : audioData.balance;
      ...
      if (switches == null) {
        ...
        for (let value of stringValue.replace(/ +/, "").split(/,/)) {
          if (value.match(/^([!~]?)switch:(\d+)$/i)) {
            let type =
              RegExp.$1.replace("!", "off").replace("~", "reverse") || "on";
